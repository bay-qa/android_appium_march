When(/^I swipe screen to the (right|left) direction$/) do |direction|
  if direction == "right"
    ...
    $driver.swipe(:start_x => 0.9, :start_y => 0.5, :end_x => 0.01, :end_y => 0.5, :duration => 1500)
    sleep 1
  end
end
...
Then(/^I should( not)? see Navigation menu$/) do |option|
  if option == nil
    fail("Expecting to see Navigation menu") unless $driver.exists{account_text} == true
  elsif option == " not"
    fail("Not expecting to see Navigation menu") unless $driver.exists{account_text} == false
  end
end
